export  const registerEmailContent = (token) => ({
 ...
 export  const resetPassEmailContent = (token) => ({
  ...
  export const vendorRegisterEmailContent = (token) => ({
    subject: "Weed-Clone Vendor Email Verification",
    text: `Click on the link below to verify your email as a vendor: ${process.env.BASE_URL}/api/v1/vendors/verify-email?token=${token}`,
    html: `
      <p>Hello,</p>
      <p>Thank you for registering as a vendor. Please verify your email using the link below:</p>
      <a href="${process.env.BASE_URL}/api/v1/vendors/verify-email?token=${token}">Verify Vendor Email</a>
      <p>This link will expire in 10 minutes.</p>
      <p>Regards,<br>Your App Team</p>
    `,
  });
