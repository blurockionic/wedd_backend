    ...
    try {
      const tagConnections = await Promise.all(
        tags.map(async (tagName) => {
          const existingTag = await postgresPrisma.tags.findUnique({ where: { tagName } });
          if (existingTag) return { id: existingTag.id };
          const newTag = await postgresPrisma.tags.create({ data: { tagName } });
          return { id: newTag.id };
        })
      );
      ...
      const newBlog = await postgresPrisma.blog.create({
        data: {
          title,content,urlTitle,coverImage,authorId,
