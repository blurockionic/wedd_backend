...
import {
  ...
  Prisma,
} from "../../prisma/generated/postgres/index.js";
...
export const getBlogs = async (req, res, next) => {
  try {
    ...
    // Map authorId to authorName for frontend
    const formattedBlogs = blogs.map(({ authorId, ...rest }) => ({
      ...rest,
      authorName: authorId,
    }));
    res.status(200).json({
...
export const addBlog = async (req, res, next) => {
  ...
  try {
    ...
    // Generate plain-text excerpt (max 200 chars)
    ...
    // Use author's display name instead of ID
...
export const getBlogCounts = async (req, res, next) => {
  try {
    ...
    const totalCount = await postgresPrisma.blog.count();
    const filteredCount = await postgresPrisma.blog.count({
      where: whereClause,
    });
...
export const updateBlog = async (req, res, next) => {
  ...
  try {
    ...
    // Also update authorId if user_name changed (optional)
    updateData.authorId = req.user.user_name;
      ...
      try {
        ...
        if (relatedBlogs.length > 0) {
          ...
          relatedBlogs = relatedBlogs.map((blog) => {
            // Add tags
            const blogTags = tagsForBlogs
              .filter((tag) => tag.blogs.some((b) => b.id === blog.id))
              .map(({ id, tagName }) => ({ id, tagName }));

            // Add counts
            const commentCount =
              commentCounts.find((c) => c.blogId === blog.id)?._count || 0;
            const likeCount =
              likeCounts.find((c) => c.blogId === blog.id)?._count || 0;
